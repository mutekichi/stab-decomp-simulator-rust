...
fn _zero_minus_i_one_state(num_qubits: usize) -> Result<StabilizerCHForm> {
    if num_qubits == 0 {
...
fn _even_parity_state(num_qubits: usize) -> Result<StabilizerCHForm> {
    let mut state = StabilizerCHForm::new(num_qubits)?;
...
fn _even_parity_phase_flipped_state(num_qubits: usize) -> Result<StabilizerCHForm> {
...
fn _construct_cat_1_state() -> Result<StabilizerDecomposedState<Scalar>> {
...
fn _construct_cat_2_state() -> Result<StabilizerDecomposedState<Scalar>> {
...
fn _construct_cat_4_state() -> Result<StabilizerDecomposedState<Scalar>> {
...
fn _construct_cat_6_state() -> Result<StabilizerDecomposedState<Scalar>> {
...
fn _project_ch_form_onto_cat_state(state: &mut StabilizerCHForm, qubits: &[usize]) -> Result<()> {
...
fn _reduce_cat_state(state: &mut StabilizerDecomposedState<Scalar>) -> Result<()> {
...
pub(crate) fn _construct_cat_state(num_qubits: usize) -> Result<StabilizerDecomposedState<Scalar>> {
    match num_qubits {
        ...
        _ => {
            let mut cat_pair =
                _construct_cat_state(num_qubits - 4)?.kron(&_construct_cat_state(6)?)?;
